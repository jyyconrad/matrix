...
export const matrixPlugin: ChannelPlugin<ResolvedMatrixAccount> = {
  ...
  mentions: {
    stripPatterns: ({ account }) => {
      // Matrix 提及模式：@user:server 格式
      ...
      const userIdPattern = escapedUserId.startsWith('@') ? escapedUserId : `@${escapedUserId}`;
      return [
        // 同时匹配普通格式和尖括号格式，支持消息中任意位置的提及
        new RegExp(`(?:^|\\s+)<?${userIdPattern}>?(?=\\s+|$)`, "gi"),
      ];
    },
    stripMentions: ({ text, patterns }) => {
      return patterns.reduce((stripped, pattern) => stripped.replace(pattern, " "), text)
        .replace(/\s+/g, " ")
        .trim();
    },
  },
